...
def test_compute_priority_score_includes_all_factors():
    ...
    assert score == pytest.approx(19.5)
...
def test_prioritize_work_orders_sorts_highest_score_first():
    low = make_order("WO-LOW", 0.1, 1, 0, 0, 0)
    high = make_order("WO-HIGH", 0.9, 5, 8, 10, 10)
    middle = make_order("WO-MID", 0.5, 4, 2, 4, 5)
...
def test_prioritize_work_orders_breaks_score_ties_by_work_order_id():
    first = make_order("WO-1", 0.5, 4, 0, 0, 0)
    second = make_order("WO-2", 0.5, 4, 0, 0, 0)
